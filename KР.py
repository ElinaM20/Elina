...
def ternary_search(arr, target):
    """
    Реализует алгоритм тернарного поиска в отсортированном массиве.
    Параметры:
    arr - отсортированный массив, в котором ведется поиск.
    target - искомое значение.
    Возвращает индекс найденного элемента или -1, если элемент не найден.
    ...
    while left <= right:
        ...
        # Если найден элемент в первой третьей точке
        if arr[third1] == target:
            return third1
        # Если найден элемент во второй третьей точке
        elif arr[third2] == target:
            return third2
        # Если искомый элемент меньше первой третей точки, сузим область поиска влево
        elif target < arr[third1]:
            right = third1 - 1
        # Если искомый элемент больше второй третей точки, сузим область поиска вправо
        elif target > arr[third2]:
            left = third2 + 1
        # Если искомый элемент находится между первыми двумя третьими точками
        else:
            left = third1 + 1
            right = third2 - 1
