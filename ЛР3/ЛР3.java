...
return new_item; }
/** ** Delete the minimal item * Complexity: O(log(n)) **/
...
this.min = findNewMin().node; }
/** ** Delete the item from the heap. * Complexity: O(log(n)) **/
public void delete(HeapItem item) {
decreaseKey(item,1+item.key);
deleteMin(); }
/** ** Meld the heap with heap2 * Complexity: O(log(n)) **/
...
for (int i = 0; i < treenumber
- 1; i++) {
if (currentNode.rank == currentNode.next.rank) {
if (treenumber
- i >= 3 && currentNode.next.rank == currentNode.next.next.rank) {
prevNode = currentNode;
currentNode = currentNode.next;
nextNode = currentNode.next; continue; }
boolean flag = nextNode == this.last;
nextNode = nextNode.next;
HeapItem tmp = link(currentNode, currentNode.next);
currentNode = tmp.node;
...
/** ** linking two heapnode with same rank to one * Complexity: O(1) ***/
...
y.setParent(x); }
return x.item; }
/** ** merging two BinomialHeap into one without linking node with same rank * Complexity: O(log(n)) ***/
...
this.size = heap.size; }
/** ** convert BinomialHeap to Array * Complexity: O(log(n)) ***/
...
currnode = currnode.next; }
return arr; }
/** ** convert Array to BinomialHeap * Complexity: O(log(n)) **
*/
...
arr[arr.length
-1].next = arr[0];
size += (int)Math.pow(2,arr[arr.length
-1].rank);
heap.last = arr[arr.length
-1];
...
return (size==0); }
/** ** Return the number of trees in the heap. * Complexity: O(log(n)) **/
public int numTrees() {
if(this.empty()){
return 0; }
int numtrees = 1;
...
//Хеш-таблица с помощью класса Hashtable
...
//Хеш-таблица с помощью класса HashMap
