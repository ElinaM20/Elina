...
node* p = new node; p->n = k;
return p; }
/** Insert Nodes in Binomial Heap */
node* BinomialHeap::Insert(node* H, node* x) {
node* H1 = Initializeheap(); x->parent = NULL; x->child = NULL; x->sibling = NULL; x->degree = 0;
H1 = x;
H = Union(H, H1);
return H; }
...
->right = x;
return x; }}
void _link(Element<T> *y, Element<T> *x) { y->left->right = y->right; y->right->left = y->left;
if (x
->child == nullptr) {
x
->child = y;
...
void _cut(Element<T> *x, Element<T> *y) { x->left->right = x->right; x->right->left = x->left;
if (y
->child == x) {
if (x
->right == x) {
...
if (k > x
->key) {
throw std::invalid_argument("new key is greater than current key"); }x->key = k;
Element<T> *y = x
->parent;
if (y != nullptr && x
->key < y
...
//Хеш-таблица, хранящая пары «ключ-значение»
